...
class StartMenu {
   ...
   */
  async createDesktopShortcut(item) {
    ...
    try {
      ...
      await window.RetroWeb.vfs.writeFile(
        fullPath,
      ...
      // Optional: Feedback/Notification is hard because StartMenu closes?
      // Actually StartMenu stays open unless we close it.
      // Let's close it to show the desktop result.
      this.close();
    } catch (err) {
      console.error("[START-MENU] Failed to create shortcut:", err);
  ...
  createAllProgramsMenu() {
    ...
    programs.forEach((item) => {
      ...
      if (item.submenu) {
        ...
        el.innerHTML +=
          ' <span style="font-size: 8px; margin-left: auto;">▶</span>';
  ...
  handleMenuItemClick(action) {
    ...
    switch (action) {
      ...
      case "logoff":
        ...
        break;
      case "minesweeper":
        this.launchApp("Minesweeper", "💣");
        break;
      case "solitaire":
        this.launchApp("Solitaire", "🃏");
        break;
      default:
    ...
    switch (name.toLowerCase()) {
      case "notepad":
        ...
        if (window.RetroWeb?.registry) {
          window.RetroWeb.registry.launch('Notepad');
          return;
        }
        break;
      case "terminal":
        ...
        // Try registry launch
        if (window.RetroWeb?.registry) {
          window.RetroWeb.registry.launch('Terminal');
          return;
        }
        break;
      case "paint":
        ...
        // Try registry launch
        if (window.RetroWeb?.registry) {
          window.RetroWeb.registry.launch('Paint');
          return;
        }
        break;
      case "calculator":
        ...
        // Try registry launch
        if (window.RetroWeb?.registry) {
          window.RetroWeb.registry.launch('Calculator');
          return;
        }
        break;
      case "file explorer":
      ...
      case "control panel":
        ...
        break;
      case "minesweeper":
        if (window.RetroWeb?.minesweeper) {
          window.RetroWeb.minesweeper.open();
          return;
        }
        // Try registry launch
        if (window.RetroWeb?.registry) {
          window.RetroWeb.registry.launch('Minesweeper');
          return;
        }
        break;
      case "solitaire":
        if (window.RetroWeb?.solitaire) {
          window.RetroWeb.solitaire.open();
          return;
        }
        // Try registry launch
        if (window.RetroWeb?.registry) {
          window.RetroWeb.registry.launch('Solitaire');
          return;
        }
        break;
    }
  ...
  openSearch() {
    ...
    const performSearch = async () => {
      ...
      resultsBody.innerHTML =
        '<tr><td colspan="3" style="padding: 10px; text-align: center;">Searching...</td></tr>';
      ...
      try {
        ...
        if (window.RetroWeb?.vfs) {
          ...
          if (results.length === 0) {
            resultsBody.innerHTML =
              '<tr><td colspan="3" style="padding: 10px; text-align: center;">No results found.</td></tr>';
          } else {
